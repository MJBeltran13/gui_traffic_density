...
# Function to plot density over time
def plot_density_over_time():
    fig, ax = plt.subplots(figsize=(6, 4))
    ax.plot(data["hour"], data["left_density"], label="Left Density", color="blue")
    ax.plot(data["hour"], data["right_density"], label="Right Density", color="red")
    ax.set_xlabel("Hour")
...
def show_statistics():
    stats = data[["left_density", "right_density"]].describe()
...
def show_correlation_heatmap():
    fig, ax = plt.subplots(figsize=(6, 4))
    sns.heatmap(processed_data.corr(), annot=True, fmt=".2f", ax=ax, cmap="coolwarm")
    ax.set_title("Feature Correlation Heatmap")

    # Embed the heatmap in Tkinter window
    canvas = FigureCanvasTkAgg(fig, master=root)
...
stats_button = tk.Button(root, text="Show Statistics", command=show_statistics)
stats_button.grid(row=7, column=0, padx=10)

# Button to show correlation heatmap
correlation_button = tk.Button(
    root, text="Show Correlation Heatmap", command=show_correlation_heatmap
)
correlation_button.grid(row=7, column=1, padx=10)
